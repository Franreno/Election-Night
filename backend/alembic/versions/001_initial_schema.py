...
def upgrade() -> None:
    ...
    op.create_index("ix_constituencies_name", "constituencies", ["name"], unique=True)
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.UniqueConstraint("constituency_id", "party_code", name="uq_constituency_party"),
        sa.CheckConstraint("votes >= 0", name="ck_votes_non_negative"),
    )
    op.create_index("ix_results_constituency_id", "results", ["constituency_id"])
