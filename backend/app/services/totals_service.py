...
def get_total_results(db: Session) -> dict:
    ...
    votes_query = (db.query(Result.party_code,
                            func.sum(
                                Result.votes).label("total_votes")).group_by(
                                    Result.party_code).all())
    ...
    ).group_by(Result.constituency_id).subquery())
