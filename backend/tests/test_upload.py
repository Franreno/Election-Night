...
class TestUploadEndpoint:
    ...
    def test_upload_diacritic_matching(self, client, db_session):
        """'Ynys Mon' (no diacritic) matches DB name 'Ynys Môn'."""
        seed_constituencies(db_session, ["Ynys M\u00f4n"])
        content = "Ynys Mon,500,C,300,L\n"
        response = client.post(
            "/api/upload",
            files={
                "file": ("r.txt", io.BytesIO(content.encode()), "text/plain")
    ...
    def test_upload_unmatched_name_is_error(self, client, db_session):
        """A name that doesn't match any DB constituency is logged as error."""
        seed_constituencies(db_session, ["Bedford"])
        content = "Nonexistent Place,500,C,300,L\n"
        response = client.post(
            "/api/upload",
            files={
                "file": ("r.txt", io.BytesIO(content.encode()), "text/plain")
